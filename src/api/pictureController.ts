...
/** listMyPictureVOByPage POST /api/picture/list/page/my/vo */
export async function listMyPictureVoByPageUsingPost(
  body: API.PictureQueryRequest,
  options?: { [key: string]: any }
) {
  return request<API.BaseResponsePagePictureVO_>('/api/picture/list/page/my/vo', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
...
/** uploadUserAvatarPicture POST /api/upload/avatar */
export async function uploadUserAvatarPictureUsingPost(
  file: File,
  options?: { [key: string]: any }
) {
  const formData = new FormData();
  formData.append('file', file); // 只需要传 file 参数

  return request<API.BaseResponseString>(
    '/api/picture/upload/avatar',
    {
      method: 'POST',
      data: formData,
      requestType: 'form', // 使用 FormData 上传文件
      ...(options || {}),
    }
  );
}
